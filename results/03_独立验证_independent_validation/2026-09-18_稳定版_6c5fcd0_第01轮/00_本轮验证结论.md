...
- 证据: 本包 `02_公开测试结果_public/6c5fcd0-official-v1/`（完整副本；原公开评测轮次现位于 `results/02_公开评测_public_eval/2026-09-18_稳定版公开评测_6c5fcd0_第01轮/`，含 `00_本轮验证结论.md`、集成 diff、构建日志、pair-swap 诊断、算法文件哈希）
...
- 证据: 本包 `04_千节点压力验证_scale_1000/<case>/`（各 3 次 log+time、stat、EQY 日志、记录 md、网表；原始目录 `/tmp/pmux_scale/results/scale_1000/6c5fcd0/`）；本目录 `千节点压力验证汇总.md/csv`
...
- 证据: 本包 `03_隐藏风格测试_blind_tests/blind01…blind08/`（README + RTL + EQY 配置/日志 + stat + 运行日志（baseline_run*.log / optimized_run*.log）+ time/RSS）；原始目录 `/tmp/pmux_blind_tests/6c5fcd0/`
...
- 算法哈希: `src/pmux_opt.cc` sha256 见上；已归档于 `02_公开测试结果_public/6c5fcd0-official-v1/pmux_opt.6c5fcd0.cc(.sha256)`
