#!/usr/bin/env bash
set -e

DIR="$(cd "$(dirname "$0")" && pwd)"

cat > "$DIR/a1_width4_bit0.v" <<'VEOF'
module a1_width4_bit0(
    input  [1:0] control,
    input  [3:0] d0, d1,
    input  [3:0] e0, e1,
    output reg [3:0] x,
    output reg [3:0] y
);
always @* begin
    x = 4'b0;
    y = 4'b0;
    case (control)
        2'd0: begin x = d0; y = e0; end
        2'd1: begin x = e0; y = d0; end
        2'd2: begin x = d1; y = e1; end
        2'd3: begin x = e1; y = d1; end
    endcase
end
endmodule
VEOF

cat > "$DIR/a2_width4_bit1.v" <<'VEOF'
module a2_width4_bit1(
    input  [1:0] control,
    input  [3:0] d0, d1,
    input  [3:0] e0, e1,
    output reg [3:0] x,
    output reg [3:0] y
);
always @* begin
    x = 4'b0;
    y = 4'b0;
    case (control)
        2'd0: begin x = d0; y = e0; end
        2'd1: begin x = d1; y = e1; end
        2'd2: begin x = e0; y = d0; end
        2'd3: begin x = e1; y = d1; end
    endcase
end
endmodule
VEOF

cat > "$DIR/a3_width8_bit2.v" <<'VEOF'
module a3_width8_bit2(
    input  [2:0] control,
    input  [7:0] d0, d1, d2, d3,
    input  [7:0] e0, e1, e2, e3,
    output reg [7:0] x,
    output reg [7:0] y
);
always @* begin
    x = 8'b0;
    y = 8'b0;
    case (control)
        3'd0: begin x = d0; y = e0; end
        3'd1: begin x = d1; y = e1; end
        3'd2: begin x = d2; y = e2; end
        3'd3: begin x = d3; y = e3; end
        3'd4: begin x = e0; y = d0; end
        3'd5: begin x = e1; y = d1; end
        3'd6: begin x = e2; y = d2; end
        3'd7: begin x = e3; y = d3; end
    endcase
end
endmodule
VEOF

cat > "$DIR/a4_half_swap.v" <<'VEOF'
module a4_half_swap(
    input  [1:0] control,
    input  [3:0] d0, d1,
    input  [3:0] e0, e1,
    input  [3:0] p,
    input  [3:0] q,
    output reg [3:0] x,
    output reg [3:0] y
);
always @* begin
    x = 4'b0;
    y = 4'b0;
    case (control)
        2'd0: begin x = d0; y = e0; end
        2'd1: begin x = e0; y = d0; end
        2'd2: begin x = d1; y = e1; end
        2'd3: begin x = p;  y = q;  end
    endcase
end
endmodule
VEOF

cat > "$DIR/a5_one_special.v" <<'VEOF'
module a5_one_special(
    input  [2:0] control,
    input  [7:0] d0, d1, d2,
    input  [7:0] e0, e1, e2,
    input  [7:0] special_x,
    input  [7:0] special_y,
    output reg [7:0] x,
    output reg [7:0] y
);
always @* begin
    x = 8'b0;
    y = 8'b0;
    case (control)
        3'd0: begin x = special_x; y = special_y; end
        3'd1: begin x = special_x; y = special_y; end

        3'd2: begin x = d0; y = e0; end
        3'd3: begin x = e0; y = d0; end

        3'd4: begin x = d1; y = e1; end
        3'd5: begin x = e1; y = d1; end

        3'd6: begin x = d2; y = e2; end
        3'd7: begin x = e2; y = d2; end
    endcase
end
endmodule
VEOF

echo "Pattern A cases created."
