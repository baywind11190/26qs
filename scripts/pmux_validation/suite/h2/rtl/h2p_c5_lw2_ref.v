// 独立参考模型（移位表达）: phase=ctrl[1:0], bank=ctrl[4:2]
// 模块名与用例相同（EQY 要求 gold/gate 顶层同名）
module h2p_c5_lw2 (
    input  wire [4:0] ctrl,
    input  wire [63:0] d,
    output wire [7:0] y
);
    wire [7:0] pre = (ctrl[4:2] == 3'd0) ? d[7:0] : (ctrl[4:2] == 3'd1) ? d[15:8] : (ctrl[4:2] == 3'd2) ? d[23:16] : (ctrl[4:2] == 3'd3) ? d[31:24] : (ctrl[4:2] == 3'd4) ? d[39:32] : (ctrl[4:2] == 3'd5) ? d[47:40] : (ctrl[4:2] == 3'd6) ? d[55:48] : (ctrl[4:2] == 3'd7) ? d[63:56] : 8'b0;
    wire [1:0] phase = ctrl[1:0];
    wire [7:0] s_amt = phase * 2;  // 移位量（bit）
    wire [7:0] rot_r = pre >> s_amt;
    wire [7:0] rot_l = pre << (8 - s_amt);
    assign y = (phase == 2'd0) ? pre
                          : (rot_r | rot_l);
endmodule
